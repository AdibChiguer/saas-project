    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-6 md:space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <div className="bg-white p-4 md:p-6 rounded-2xl border shadow-sm flex flex-col items-center justify-center">
          ...
          <p className="text-lg md:text-2xl font-black text-zinc-900">{factures.reduce((s, f) => s + f.totalHt, 0).toLocaleString()} €</p>
        </div>
        <div className="bg-white p-4 md:p-6 rounded-2xl border shadow-sm flex flex-col items-center justify-center border-l-4 border-l-emerald-500">
          ...
          <p className="text-lg md:text-2xl font-black text-emerald-600">{factures.filter(f => f.statut === 'payee').length}</p>
        </div>
        <div className="bg-white p-4 md:p-6 rounded-2xl border shadow-sm flex flex-col items-center justify-center border-l-4 border-l-amber-500">
          ...
          <p className="text-lg md:text-2xl font-black text-amber-600">{factures.filter(f => f.statut === 'generee' || f.statut === 'envoyee').length}</p>
        </div>
        <div className="bg-white p-4 md:p-6 rounded-2xl border shadow-sm flex flex-col items-center justify-center border-l-4 border-l-red-500">
          ...
          <p className="text-lg md:text-2xl font-black text-red-600">{factures.filter(f => f.statut === 'en_retard').length}</p>
        </div>
      </div>
              ...
              {loading ? (
                ...
                factures.map((f) => (
                  <tr key={f.id} className="border-b hover:bg-zinc-50 transition-colors">
                    ...
                    <td className="p-4 text-sm text-zinc-600">{new Date(f.dateEmission).toLocaleDateString('fr-FR')}</td>
                    <td className="p-4 text-sm text-zinc-600">{new Date(f.dateEcheance).toLocaleDateString('fr-FR')}</td>
                    <td className="p-4 font-bold text-sm text-zinc-900 whitespace-nowrap">{f.totalTtc.toLocaleString()} €</td>
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      <button 
                        ...
                        className="text-emerald-600 hover:text-emerald-700 p-2 rounded-full hover:bg-emerald-50 transition-colors flex items-center justify-center"
                      >
