...
export function Sidebar({ isCollapsed, setIsCollapsed }) {
  ...
  const menuItems = [
    ...
    { icon: Users, label: t("common.clients"), href: "/dashboard/clients" },
    { icon: FileText, label: t("common.quotes"), href: "/dashboard/invoices" },
    { icon: Receipt, label: t("common.invoices"), href: "/dashboard/receipts" },
  ...
  return (
    <>
      ...
      <aside className={cn(
        ...
        <div className={cn(
          "px-4 mb-4",
          isCollapsed ? "flex flex-col items-center gap-2" : "flex items-center justify-around bg-slate-800/30 mx-4 py-2 rounded-xl border border-slate-800"
        )}>
          {isCollapsed ? (
            <button 
              onClick={() => changeLanguage(locale === "fr" ? "en" : locale === "en" ? "nl" : "fr")}
              className="w-10 h-10 flex items-center justify-center bg-slate-800 rounded-lg hover:text-white transition-colors"
            ...
            ["fr", "en", "nl"].map((lang) => (
              <button
                ...
                className={cn(
                  ...
                  locale === lang ? "bg-blue-600 text-white shadow-lg" : "hover:text-slate-200"
                )}
              >
                {lang}
              </button>
        ...
        <div className="p-4 border-t border-slate-800">
          ...
          >
            ...
            {!isCollapsed && <span className="font-medium truncate">{t("common.logout")}</span>}
          </button>
        </div>
      </aside>
    </>
