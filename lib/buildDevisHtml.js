...
// ── main builder ──────────────────────────────────────────────────────────────
export function buildDevisHtml({ devis, workLogs, param, user, locale = "fr" }) {
  const t = (key) => {
  ...
  // ... (rest of the code stays mostly the same, but using t() for labels)
  const DAY_COLS_MAP = {
    fr: ["LU", "MA", "ME", "JE", "VE", "SA", "DI"],
    en: ["MO", "TU", "WE", "TH", "FR", "SA", "SU"],
    nl: ["MA", "DI", "WO", "DO", "VR", "ZA", "ZO"]
  };
  const DAY_COLS = DAY_COLS_MAP[locale] || DAY_COLS_MAP.fr;
  ...
  const linesHtml = devis.lignes
    .map(
      ...
      <tr>
        ...
        <td>${esc(l.unite)}</td>
        <td style="text-align:right">${fmt(l.prixUnitaire, currentLocale)}</td>
        <td style="text-align:right">${fmt(l.quantite, currentLocale)}</td>
        <td style="text-align:right">${fmt(l.montantHt, currentLocale)}</td>
      </tr>`
    )
    .join("");
  ...
  if (workLogs.length > 0) {
    ...
    for (const wl of workLogs) {
      ...
      const name    = wlOwner?.name       || user?.name;
      const sofi    = wlOwner?.Sofinummer || user?.Sofinummer;
...
<div class="page">
  ...
  <!-- HEADER -->
  <div class="header">
    <div class="devis-title">${t("pdf.quote")} n° ${esc(devis.numero)}</div>
    <div>
  ...
  <div class="meta-receiver">
    <div class="meta-box">
      <table>
        <tr><td>${t("pdf.date")}</td>  <td>${fmtDate(devis.dateEmission, currentLocale)}</td></tr>
        <tr><td>${t("pdf.reference")}</td>      <td>${esc(devis.numero)}</td></tr>
        <tr><td>${t("pdf.contact")}</td> <td>${esc(user?.name)}</td></tr>
        <tr><td>${t("pdf.telephone")}</td>      <td>${esc(user?.telephone)}</td></tr>
        <tr><td>${t("pdf.email")}</td>      <td>${esc(user?.email)}</td></tr>
        ${user?.kvknr ? `<tr><td>KVK nr.</td><td>${esc(user.kvknr)}</td></tr>` : ""}
      </table>
    </div>
    <div class="receiver">
      <div class="label">${t("pdf.recipient")}</div>
      ...
      <p>
        ...
        ${devis.client?.kvknr     ? `<br>KVK&nbsp;: ${esc(devis.client.kvknr)}`  : ""}
        ${devis.client?.btwnr     ? `<br>BTW&nbsp;: ${esc(devis.client.btwnr)}`  : ""}
      </p>
    </div>
  </div>
  ...
  <div class="extra-info">
    <div class="label">${t("dashboard.generate_quote")}</div>
    <p>${esc(devis.notes ?? "")}</p>
  </div>
  ...
  <div class="totals">
    <div class="totals-box">
      <div class="total-row">
        <span>${t("pdf.total_excl_vat")}</span>
        <span>${fmt(devis.total, currentLocale)} €</span>
      </div>
      <div class="total-row">
        <span>${t("pdf.vat")} (0%)</span>
        <span>0,00 €</span>
      </div>
      <div class="total-row grand">
        <span>${t("pdf.total_incl_vat")}</span>
        <span>${fmt(devis.total, currentLocale)} €</span>
      </div>
    </div>
  </div>
